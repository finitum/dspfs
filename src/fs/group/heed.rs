...
impl GroupStore for HeedGroupStore {
    fn add_file(&mut self, user: &PublicUser, file: File) -> Result<()> {
        ...
        let mut tree = self.filetrees.get(&wtxn, user).context("Error accessing the db")?.unwrap_or_else(FileTree::new);

        tree.insert(file.path.clone(), file)?;

        self.filetrees.put(&mut wtxn, user, &tree).context("error saving to the db")?;
    ...
    fn get_file(&self, hash: Hash) -> Result<Option<File>> {
        ...
        Ok(self.files.get(&rtxn, &hash).context("error getting hash from db")?)
    }

    fn delete_file(&mut self, user: &PublicUser, file: &File) -> Result<()> {
        let mut wtxn = self.env.write_txn()?;

        if let Some(mut tree) = self.filetrees.get(&wtxn, user).context("Error accessing the db")? {
            tree.delete(&file.path, false);

            self.filetrees.put(&mut wtxn, user, &tree).context("error saving to the db")?;
        }

        if let Some(mut file) = self.files.get(&wtxn, &file.hash).context("Error accessing the db")? {
